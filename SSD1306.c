/**
  ...
  ******************************************************************************
  *
  * This is modified file of an OLED driver for nrf52 purposes.
  * Only I2C (TWI) connection is available.
  * You can use SSD1306 or SH1106 display.
  *
  *
  * modified by: Bartosz Bryk, January 2019
  */
...
static uint8_t  _i2caddr = SSD1306_I2C_ADDR;
...
static void ssd1306_write_byte(uint8_t chData, uint8_t chCmd)
{
	if(chCmd == SSD1306_CMD){
	    uint8_t dta_send[] = {0x00, chData};
	    nrf_drv_twi_tx(&m_twi_master, _i2caddr, dta_send, 2, false);
	}else{
	    uint8_t dta_send[] = {0x40, chData};
	    nrf_drv_twi_tx(&m_twi_master, _i2caddr, dta_send, 2, false);
	}
...
void ssd1306_refresh_gram(void)
{
	...
	for (i = 0; i < 8; i ++) {
		...
		for (j = 1; j < 129; j ++) {
			BufTmp[j] = s_chDispalyBuffer[j-1][i];

		}
	    nrf_drv_twi_tx(&m_twi_master, _i2caddr, BufTmp, sizeof(BufTmp), false);
	}
}
...
void ssd1306_fill_in_screen(void)
{
	uint8_t i, j;
	uint8_t chFill = 0xFF;
	for (i = 0; i < 8; i ++) {
...
void ssd1306_display_num_ext(uint8_t chXpos, uint8_t chYpos, uint32_t chNum, uint8_t chLen, uint8_t chSize, uint8_t chMode)
{
	uint8_t i;
	uint8_t chTemp, chShow = 0;
	...
	for(i = 0; i < chLen; i ++) {
		...
		if(chShow == 0 && i < (chLen - 1)) {
			if(chTemp == 0) {
				ssd1306_display_char(chXpos + (chSize / 2) * i, chYpos, ' ', chSize, chMode);
				continue;
			} else {
				chShow = 1;
			}
		}
	 	ssd1306_display_char(chXpos + (chSize / 2) * i, chYpos, chTemp + '0', chSize, chMode);
	}
}
...
//    ssd1306_write_byte(0xFF, SSD1306_CMD);// Set SEG Output Current Brightness (ppi - im wieksza liczyba tym wiekszy kontrast)
    ssd1306_write_byte(0x1F, SSD1306_CMD);// Set SEG Output Current Brightness
	...
	nrf_delay_ms(100);
	ssd1306_write_byte(0xC8, SSD1306_CMD);	 // flip display (tdz)
	ssd1306_write_byte(0xA0, SSD1306_CMD);	 // flip display (tdz)
	ssd1306_write_byte(0xAF, SSD1306_CMD);	 //Set Display On
	nrf_delay_ms(100);
	ssd1306_clear_screen();
	...
	ssd1306_write_byte(0xAD, SSD1306_CMD);
	ssd1306_write_byte(0x8B, SSD1306_CMD);
	ssd1306_write_byte(0xC8, SSD1306_CMD);
	ssd1306_write_byte(0xD3, SSD1306_CMD);
	ssd1306_write_byte(0x0, SSD1306_CMD);
